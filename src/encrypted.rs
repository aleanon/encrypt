...
impl<T> Encrypted<T> 
    ...
    pub(crate) fn new(mut key_salt_pair: KeySaltPair<T>, data: Vec<u8>, aad: impl AsRef<[u8]>) -> Result<Self, T::Error> {
        ...
        instance.encrypt(key_salt_pair.key(), aad)?; 

        Ok(instance)
    }

    pub(crate) fn new_without_salt(key: &impl Key, data: Vec<u8>, aad: impl AsRef<[u8]>) -> Result<Self, T::Error> {
        let mut instance = Self {
            data,
            salt: None,
            nonce_bytes: NonceBytes::<T>::default(),
        ...
        instance.encrypt(key, aad)?;

        Ok(instance)
    }
    

    pub(crate) fn encrypt(&mut self, key: &impl Key, aad: impl AsRef<[u8]>) -> Result<(), T::Error> {
        let Ok(nonce_bytes) = Algo::<T>::encrypt(&mut self.data, key, aad) else {
            self.data.zeroize();
    ...
    pub fn decrypt_with_secret(&mut self, secret: impl AsRef<[u8]>, aad: impl AsRef<[u8]>) -> Result<T, T::Error> {
        ...
        self.decrypt_with_key(&key, aad)        
    }

    pub fn decrypt_with_key(&mut self, key: &impl Key, aad: impl AsRef<[u8]>) -> Result<T, T::Error> {        
        let (result, data_length) = {
            let decrypted = Algo::<T>::decrypt(self.data.as_mut_slice(), key, &self.nonce_bytes, aad.as_ref())?;
